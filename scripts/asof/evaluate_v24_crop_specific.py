"""
v24 evaluation: crop-specific models on the agronomically fixed dataset.

Improvements vs v23:
  - dataset has working soil_moisture (root-zone) + GDD + heat/cold stress;
  - dead columns (region_id, district_id, till_type, names, geojson) excluded;
  - low-yield anomalies (target < 1 t/ha) dropped for train and eval, since
    they otherwise dominate MAPE and are not the crop signal we model;
  - per-crop models (wheat_combined, sunflower) plus all_crops reference.

Models:
  - v24_full     : CatBoost with field_id.
  - v24_no_field : CatBoost without field_id (honest for new fields).
  - cropwise     : external benchmark on identical rows.

Validation: walk-forward by year.

Outputs:
  models_v2/asof_comparison/asof_results_v24_crop_specific.csv
  models_v2/asof_comparison/asof_predictions_v24_crop_specific.csv
  reports/V24_CROP_SPECIFIC_FINAL_RU.md
"""
...
def main() -> None:
    ...
    for scenario, crop_filter in SCENARIOS:
        ...
        for tag in ASOF_TAGS:
            ...
            df["pred_v24_full"] = walk_forward_predict(df, use_field_id=True)
            df["pred_v24_no_field"] = walk_forward_predict(df, use_field_id=False)
            fair = df[df["pred_v24_full"].notna()].copy()
            rows.append(evaluate_block(df, scenario, tag, "v24_full", "pred_v24_full"))
            rows.append(evaluate_block(df, scenario, tag, "v24_no_field", "pred_v24_no_field"))
            rows.append(evaluate_block(fair, scenario, tag, "cropwise", "cropwise_asof_t_ha"))
            tmp = df[
                ["field_id", "year", "standard_name", "target_yield_t_ha",
                 "cropwise_asof_t_ha", "pred_v24_full", "pred_v24_no_field"]
            ].copy()
    ...
    res.to_csv(COMP_DIR / "asof_results_v24_crop_specific.csv", index=False)
    preds.to_csv(COMP_DIR / "asof_predictions_v24_crop_specific.csv", index=False)

    show = res.copy()
    for col in ["r2", "wcy_r2", "rmse", "mae", "mape"]:
        show[col] = show[col].map(lambda x: round(float(x), 3))
    md = ["# v24 crop-specific (agronomic) results\n\n"]
    md.append("Dataset: v24 agro (soil_moisture fixed + GDD + heat/cold stress), target >= 1 t/ha.\n\n")
    md.append("Models: `v24_full` (with field_id), `v24_no_field`, `cropwise` (benchmark, same rows).\n\n")
    md.append(show[["scenario", "asof", "model", "n", "mape", "r2", "wcy_r2", "rmse", "mae"]].to_markdown(index=False))
    md.append("\n")
    (REPORTS / "V24_CROP_SPECIFIC_FINAL_RU.md").write_text("".join(md), encoding="utf-8")
