"""
Sprint 5.2 — Train as-of CatBoost on v4 (v3 + NDVI anomaly) and compare:
    v3   (Open-Meteo backed)
    v4   (v3 + NDVI anomaly relative to per-field history)
    Cropwise

Outputs:
  - models_v2/asof_comparison/asof_results_v4.csv
  - models_v2/asof_comparison/per_row_predictions_v4.csv
  - reports/figures/asof_mape_v3_vs_v4.png
  - reports/figures/asof_rmse_v3_vs_v4.png
...
def run_scenario(scenario: str, crop_filter: list[str] | None) -> tuple[pd.DataFrame, pd.DataFrame]:
    print(f"\n{'='*80}\nSCENARIO {scenario} — v3 vs v4 vs Cropwise\n{'='*80}")
    rows, pred_rows = [], []
    ...
    for tag in AS_OF_TAGS:
        ...
        if not v4_path.exists():
            print(f"  Missing {v4_path}, skip"); continue
        ...
        if len(df_v4) < 50:
            print(f"  {tag}: skip ({len(df_v4)} rows)"); continue

        oof_v3 = walk_forward_oof(df_v3)
        ...
        keys = ["field_id", "year"]
        merged = df_v4.merge(df_v3[keys + ["ml_v3"]], on=keys, how="inner")
        ...
        cmp_ml = merged.dropna(subset=["ml_v3", "ml_v4"])
        cmp_all = merged.dropna(subset=["ml_v3", "ml_v4", "cropwise_asof_t_ha"])
        ...
        m_v3_a = metrics(cmp_all["target_yield_t_ha"].values, cmp_all["ml_v3"].values) if len(cmp_all) else {}
        ...
        rows.append({
            ...
            "v3_a_mape": m_v3_a.get("mape", float("nan")),
...
def plot_curves(summary: pd.DataFrame) -> dict[str, Path]:
    ...
    for cols, ylabel, fname in [
        (("v3_mape", "v4_mape", "cw_mape"), "MAPE, %", "asof_mape_v3_vs_v4.png"),
        (("v3_rmse", "v4_rmse", "cw_rmse"), "RMSE, t/ha", "asof_rmse_v3_vs_v4.png"),
    ]:
        plt.figure(figsize=(9, 5.5))
        for sc in summary["scenario"].unique():
            s = summary[summary["scenario"] == sc].sort_values("asof_tag")
            plt.plot(s["asof_label"], s[cols[0]], marker="o", lw=2, label=f"v3 — {sc}")
            plt.plot(s["asof_label"], s[cols[1]], marker="^", lw=2.5, label=f"v4 (anomaly) — {sc}")
        ...
        plt.title(f"NDVI anomaly effect ({ylabel.split(',')[0]})")
...
def main():
    print("=" * 80)
    print("AS-OF v3 vs v4 (NDVI anomaly) vs Cropwise — Sprint 5.2")
    print("=" * 80)
    summaries, preds = [], []
    for sc, cf in [
        ("sunflower", ["sunflower"]),
    ...
    summary.to_csv(COMP_DIR / "asof_results_v4.csv", index=False)
    pred.to_csv(COMP_DIR / "per_row_predictions_v4.csv", index=False)
    paths = plot_curves(summary)

    md = ["# Sprint 5.2 — NDVI anomaly: v3 → v4 vs Cropwise\n\n"]
    md.append("_Generated by `scripts/asof/train_compare_asof_v4.py`_\n\n")
    md.append("## What changed in v4\n")
    md.append("Added 6 features that compare current NDVI to the **per-field historical baseline**\n")
    md.append("(mean NDVI for the same field at the same DOY ±7d, over years strictly < target year):\n\n")
    md.append("- `ndvi_anom_mean_asof` — average anomaly over season-to-date\n")
    md.append("- `ndvi_anom_max_asof` — best deviation above baseline\n")
    md.append("- `ndvi_anom_min_asof` — worst deviation below baseline (stress signal)\n")
    md.append("- `ndvi_anom_last_value_asof` — current state vs baseline\n")
    md.append("- `ndvi_anom_last30d_mean_asof` — recent month vs baseline (trend)\n")
    md.append("- `ndvi_baseline_n_years` — confidence in baseline (how many history years)\n\n")
    md.append("This breaks the limit where the model could only memorise per-field absolute NDVI levels — it now sees explicit relative-to-self deviations.\n\n")
    md.append("## Results\n\n")
    md.append(summary.round(3).to_markdown(index=False))
    md.append("\n\n")
    for k, p in paths.items():
        md.append(f"### {k}\n\n![{k}]({p.relative_to(REPORTS).as_posix()})\n\n")
    out_md = REPORTS / "asof_v4_results.md"
    out_md.write_text("".join(md), encoding="utf-8")
    print("\n" + "=" * 80); print("FINAL"); print("=" * 80)
    print(summary.round(3).to_string(index=False))
    print(f"\nSaved {COMP_DIR / 'asof_results_v4.csv'}")
